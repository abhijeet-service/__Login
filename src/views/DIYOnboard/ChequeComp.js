...
class ChequeComp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      files: [],
  ...
  onSaveChangesClick = () => {
    let edit_details = {
      job_id: this.props.jobID,
      doc_name: this.props.chequeVerifyDocResponse["document_type"],
    };
    this.setState(
      {
        saveDisabled: true,
  ...
  render() {
    let chequeVerifyDocResponse = this.props.chequeVerifyDocResponse;
    ...
    const sideList = (side) => (
      <div className="right-drawer">
        ...
        {Object.keys(this.props.chequeVerifyDocResponse).length !== 0 ? (
          <>
            <Table>
              <TableBody>
                {Object.keys(chequeVerifyDocResponse["document_details"]).map(
                  function(key) {
                    return (
                      <>
                        {typeof chequeVerifyDocResponse["document_details"][
                          key
                        ] === "string" ? (
                          <TableRow>
                            ...
                            <TableCell className="formDetails">
                              {that.state.saveDisabled ? (
                                <Typography className="table-typo">
                                  {
                                    chequeVerifyDocResponse["document_details"][
                                      key
                                    ]
                                  }
                                </Typography>
      ...
      <Grid container spacing={2}>
            ...
            <DropzoneArea
              ...
              dropzoneText={
                this.state.files.length === 0
                  ? "Click here to upload cancelled Cheque"
                  : "Document uploaded"
              }
              acceptedFiles={[
                "application/json",
        ...
        <Grid item lg={6} md={6} xl={6} xs={12}>
          <center>
            {Object.keys(this.props.chequeVerifyDocResponse).length === 0 ? (
              <>
                {this.props.errorChequeVerifyResponse ? (
                  <div className="diy-right-side-div">
                    ...
                    <Typography className="diy-right-side-failed-message">
                      {!this.props.errorChequeVerifyResponseMsg["error"] ? (
                        "Failed"
                      ) : (
                        <>{this.props.errorChequeVerifyResponseMsg["error"]}</>
                      )}
                  ...
                  <>
                    {this.props.enableChequeVerifyLoader ? (
                      <div className="diy-right-side-div">
                        <Typography
              ...
              <div className="diy-right-side-content-div">
                <Grid container justify="center" alignItems="center">
                  <Grid item xs={12} lg={12} sm={12} xl={12}>
                    <div>
                      {this.props.chequeVerifyDocResponse["document_status"] ? (
                        <>
                          {this.props.chequeVerifyDocResponse[
                            "document_status"
                    ...
                    <Typography>
                      ...
                      <a
                        href={
                          this.props.chequeVerifyDocResponse["document_link"]
                        }
                        target="_blank"
                        className="link"
                        ...
                        disableRipple
                      >
                        {this.props.chequeVerifyDocResponse["document_type"]}
                      </a>
                    </Typography>
                    <Link
                      href="#"
                    ...
                    <Typography
                      className={
                        (this.props.chequeVerifyDocResponse["status"] === 0 &&
                          "bad-confidence-score-typo") ||
                        (this.props.chequeVerifyDocResponse["status"] === 1 &&
                          "medium-confidence-score-typo") ||
                        (this.props.chequeVerifyDocResponse["status"] === 2 &&
                          "good-confidence-score-typo")
                      }
                    >
                      Extraction Score :{" "}
                      {this.props.chequeVerifyDocResponse["confidence_score"]}
                    </Typography>
                    <Drawer
                      anchor="right"
